...
class PairedSentenceDataset(Dataset):
    ...
    def __init__(self, data_path:str, maxSeqLen:int, tokz, **kwargs):
        ...
        '''
        super(PairedSentenceDataset, self).__init__(**kwargs)
        self.data_path = data_path
        self.maxSeqLen = maxSeqLen
        self.tokz = tokz
        self.data_and_label = PairedSentenceDataset.make_dataset(data_path, maxSeqLen, tokz)
        
    @staticmethod
    def make_dataset(data_path:str, maxSeqLen:int, tokz):
        '''
        Detailed: 
...
if __name__ == '__main__':
    ...
    dataset = PairedSentenceDataset('./data/merged_qq_train.csv', 60, tokz)
    data_sampelr = RandomSampler(dataset)
    data_iter = DataLoader(dataset=dataset, batch_size=5, sampler=data_sampelr, 
                           collate_fn=PadBatch(tokz.pad_token_id))
